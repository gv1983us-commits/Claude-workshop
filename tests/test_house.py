...
CANONIZED = {"claude.bec", "claude.mpaa"}
...
class ClaudeVoiceHouseTests(unittest.TestCase):
    ...
    def test_corpus_is_exactly_two_of_six(self) -> None:
        ...
        self.assertEqual(canon["completed_count"], 2)
        ...
        for artifact_id in EXPECTED_IDS - CANONIZED:
    ...
    def test_mpaa_record_preserves_its_large_structure_and_limits(self) -> None:
        corpus = json.loads(TECHNICAL_ARTIFACTS_JSON.read_text(encoding="utf-8"))
    ...
    def test_human_surfaces_present_two_polished_artifacts(self) -> None:
        ...
        self.assertIn("Остальные четыре артефакта", readme)
        self.assertNotIn("MPAA)** | ожидает индивидуального прохода", corpus_text)

    def test_machine_entry_blocks_mpaa_overclaim(self) -> None:
